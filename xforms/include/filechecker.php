...
xoops_loadLanguage('filechecker', 'xforms');
    ...
    public static function getFileStatus($file_path, $original_file_path = null, $redirectFile)
    {
        ...
        if (is_null($original_file_path)) {
            if (self::fileExists($file_path)) {
                ...
                $path_status .= "$file_path (" . _FC_XFORMS_AVAILABLE . ") ";
            } else {
                $path_status = "<img src='$pathIcon16/0.png' >";
                $path_status .= "$file_path (" . _FC_XFORMS_NOTAVAILABLE . ") ";
            }
        } else {
            if (self::compareFiles($file_path, $original_file_path)) {
                ...
                $path_status .= "<button class='submit' onClick='this.form.submit();'>" . _FC_XFORMS_CREATETHEFILE . "</button>";
                $path_status .= "</form>";
            }
        }
...
switch ($op) {
    case "copyfile":
        ...
        $msg = (FileChecker::copyFile($original_file_path, $file_path)) ? _FC_XFORMS_FILECOPIED : _FC_XFORMS_FILENOTCOPIED;
