...
//$xformsHelper = Helper::getHelper(XFORMS_DIRNAME);
...
//define('XFORMS_UPLOAD_PATH', XOOPS_UPLOAD_PATH  . '/' . XFORMS_DIRNAME); // WITHOUT Trailing slash
//define('XFORMS_UPLOAD_URL', XOOPS_UPLOAD_URL . '/' . XFORMS_DIRNAME); // WITHOUT Trailing slash

//include_once $xformsHelper->path('include/functions.php');
...
$xforms_isAdmin     = $xformsHelper->isUserAdmin();
...
if (!$uploadChecked) {
    ...
    if ($prevUploadPath != $currUploadPath) {
        ...
        if (!is_dir(XFORMS_UPLOAD_PATH)) {
            ...
            if (!@mkdir(XFORMS_UPLOAD_PATH) && !is_dir(XFORMS_UPLOAD_PATH)) {
                throw new \RuntimeException('The directory ' . XFORMS_UPLOAD_PATH . ' could not be created.');
            }
            umask($oldumask);
        }
...
$uploadFolders = array(
    XFORMS_UPLOAD_PATH,
    XFORMS_UPLOAD_PATH . '/images',
    XFORMS_UPLOAD_PATH . '/images/thumbnails'
);

$copyFiles = array(
    XFORMS_UPLOAD_PATH . '/images/category',
    XFORMS_UPLOAD_PATH . '/images/thumbnails'
);
