...
switch ($op) {
    case 'show': /*Show the report in the page*/
        ...
        if ((!$form = $xformsFormsHandler->get($formId)) || $form->isNew()) {
            $xformsHelper->redirect("admin/{$thisFileName}", XformsConstants::REDIRECT_DELAY_MEDIUM, _AM_XFORMS_FORM_NOTEXISTS);
        } elseif (XformsConstants::DO_NOT_SAVE_IN_DB == $form->getVar('form_save_db')) {
            $xformsHelper->redirect("admin/{$thisFileName}", XformsConstants::REDIRECT_DELAY_MEDIUM, _AM_XFORMS_FORM_NOTSAVE);
        }
        ...
        $moduleAdmin->displayNavigation($thisFileName);
        $moduleAdmin->addItemButton(_AM_XFORMS_REPORT_PURGE, "{$thisFileName}?op=purge", 'delete');
            ...
            */
            ...
            if (is_array($uDataValue)) {
                switch ($data['ele_type']) {
                    ...
                    case 'upload':
                    case 'uploadimg':
                        echo "    <td{$border}>\n" . "      <a href='" . $xformsHelper->url("file.php?f={$uDataValue['file']}&fn={$uDataValue['name']}") . "'>{$uDataValue['name']}</a>\n"
                             . "   </td>\n";
                        break;
    ...
    case 'purge_do':
        if ($ok) {
            ...
            if (!$xoopsSecurity->check()) {
                $xformsHelper->redirect("admin/{$thisFileName}", XformsConstants::REDIRECT_DELAY_MEDIUM, implode('<br>', $xoopsSecurity->getErrors()));
            }
            // ok - delete reports
            xoops_cp_header();
            ...
            if ($numItems > 0) {
                $xformsHelper->redirect("admin/{$thisFileName}", XformsConstants::REDIRECT_DELAY_MEDIUM, sprintf(_AM_XFORMS_RPT_PURGE_DELETED, (int)$numItems));
            } elseif (0 === $numItems) {
                $xformsHelper->redirect("admin/{$thisFileName}", XformsConstants::REDIRECT_DELAY_MEDIUM, _AM_XFORMS_RPT_PURGE_NOTHING);
            } else {
                $xformsHelper->redirect("admin/{$thisFileName}", XformsConstants::REDIRECT_DELAY_MEDIUM, _AM_XFORMS_RPT_PURGE_ERR);
            }
        } else {
            xoops_cp_header();
            ...
            xoops_confirm(array('op' => 'purge_do', 'purge_date' => $purgeDate, 'ok' => XformsConstants::CONFIRM_OK), $thisFileName, sprintf(_AM_XFORMS_REPORT_CONFIRM_DELETE, $theDate));
        }
        break;
    ...
    case 'export-horiz':
        ...
        switch ($format) {
            case 'h': //html
                ...
                $dl->_template = $xformsHelper->path('templates/admin/xforms_export_hh.tpl');
                $dl->_ext      = 'html';
                $dl->_type     = 'horiz';
                $dl->_nl       = '<br>';
                $dl->_mime     = 'text/html';
            ...
            default:
                ...
                $dl->_template = $xformsHelper->path('templates/admin/xforms_export_ch.tpl');
                $dl->_ext      = 'csv';
                $dl->_type     = 'horiz';
                $dl->_nl       = "\n";
                $dl->_mime     = 'text/csv';
        ...
        foreach ($uData as $key => $reportData) {
            ...
            if (is_array($reportDataValue)) {
                switch ($reportData['ele_type']) {
                    ...
                    case 'upload':
                    case 'uploadimg':
                        $rptEleValue = "<a href='" . $xformsHelper->url("file.php?f={$reportDataValue['file']}&fn={$reportDataValue['name']}") . "'>{$reportDataValue['name']}</a>";
                        break;
                    case 'checkbox':
                    case 'country':
                    case 'date':
    ...
    case 'export-vert':
        ...
        switch ($format) {
            case 'h': //html
                $dl->_template = $xformsHelper->path('templates/admin/xforms_export_hv.tpl');
                $dl->_ext      = 'html';
                $dl->_type     = 'vert';
                $dl->_nl       = '<br>';
                $dl->_mime     = 'text/html';
            ...
            default:
                $xformsTpl->register_outputfilter('xformsUndoHtmlEntities');
                $dl->_template = $xformsHelper->path('templates/admin/xforms_export_cv.tpl');
                $dl->_ext      = 'csv';
                $dl->_type     = 'vert';
                $dl->_nl       = "\n";
                $dl->_mime     = 'text/csv';
        ...
        for ($i = 0; $i < $uDataCount; ++$i) {
            ...
            if (is_array($uDataValue)) {
                switch ($uData[$i]['ele_type']) {
                    ...
                    case 'upload':
                    case 'uploadimg':
                        $tplElements['ele_value'] = "<a href='" . $xformsHelper->url("file.php?f={$uDataValue['file']}&fn={$uDataValue['name']}") . "'>{$uDataValue['name']}</a>";
                        break;
    ...
    default: /* Show list of forms with reports */
        ...
        $perpage = (int)$xformsHelper->getConfig('perpage'); // get number of items to show per page
        ...
        if ($forms) {
            ...
            foreach ($forms as $f) {
                if ($showAll || $f->isActive()) {
                    ...
                    echo "    <tr>\n" . "      <td class='{$cssClass} center'>{$id}</td>\n" . "      <td class='{$cssClass}'>\n" . "        <a target='_blank' href='"
                         . $xformsHelper->url("index.php?form_id={$id}") . "'>" . $f->getVar('form_title') . "</a><br>\n" . '        ' . $f->getVar('form_desc', 's') . "\n" . "      </td>\n"
                    ...
                    if (0 !== (int)$f->getVar('form_save_db')) {
                        echo "        <a href='{$thisFileName}?op=show&form_id={$id}'>" . "<img src='{$mypathIcon16}/rptsee.png' class='tooltip floatcenter1' title='" . _AM_XFORMS_SHOW_REPORT
                             . "' alt='" . _AM_XFORMS_SHOW_REPORT . "'>\n" . "</a>\n" . "        <a href='{$thisFileName}?op=export-horiz&format=c&form_id={$id}'>"
                             . "<img src='{$mypathIcon16}/rpthorizc.png' class='tooltip floatcenter1' title='" . _AM_XFORMS_RPT_EXPORT_CH . "' alt='" . _AM_XFORMS_RPT_EXPORT_CH . "'>" . "</a>\n"
                             . "        <a href='{$thisFileName}?op=export-horiz&format=h&form_id={$id}'>" . "<img src='{$mypathIcon16}/rpthor.png' class='tooltip floatcenter1' title='"
                             . _AM_XFORMS_RPT_EXPORT_HH . "' alt='" . _AM_XFORMS_RPT_EXPORT_HH . "'>" . "</a>\n" . "      <a href='{$thisFileName}?op=export-vert&format=c&form_id={$id}'>"
                             . "<img src='{$mypathIcon16}/rptvertc.png' class='tooltip floatcenter1' title='" . _AM_XFORMS_RPT_EXPORT_CV . "' alt='" . _AM_XFORMS_RPT_EXPORT_CV . "'>" . "</a>\n"
                             . "        <a href='{$thisFileName}?op=export-vert&format=h&form_id={$id}'>" . "<img src='{$mypathIcon16}/rptvert.png' class='tooltip floatcenter1' title='"
                             . _AM_XFORMS_RPT_EXPORT_HV . "' alt='" . _AM_XFORMS_RPT_EXPORT_HV . "'>" . "</a>\n";
                    } else {
                        echo '        <b>' . _AM_XFORMS_FORM_NOTSAVE . "</b>\n" . "        <a href='" . $GLOBALS['xoops']->buildUrl('/modules/xforms/admin/main.php',
                                                                                                                                    array('op' => 'edit', 'form_id' => $id)) . "'>" . "<img src='"
                             . Admin::iconUrl('edit.png', '16') . "' title='" . _AM_XFORMS_ACTION_EDITFORM . "' alt='" . _AM_XFORMS_ACTION_EDITFORM . "'>" . "</a>\n";
                    }
                    echo "    </tr>\n";
                }
            }
            ...
            echo "<fieldset><legend class='bold' style='color: #900;'>" . _AM_XFORMS_STATUS_INFORMATION . "</legend>\n" . "<div class='pad7'>\n" . "  <div class='center'>\n"
                 . "    <img src='{$mypathIcon16}/active.gif'>&nbsp;" . _AM_XFORMS_STATUS_ACTIVE . '&nbsp; &nbsp; &nbsp;' . "    <img src='{$mypathIcon16}/inactive.gif'>&nbsp;"
                 . _AM_XFORMS_STATUS_INACTIVE . '&nbsp; &nbsp; &nbsp;' . "    <img src='{$mypathIcon16}/expired.gif'>&nbsp;" . _AM_XFORMS_STATUS_EXPIRED . "\n" . "  </div>\n" . "</div>\n"
                 . "</fieldset>\n";
        } else {
