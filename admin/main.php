...
switch ($op) {
    ...
    default:
        ...
        $moduleAdmin->addItemButton(_AM_XFORMS_NEW, 'main.php' . '?op=edit', 'add');
        $moduleAdmin->displayButton('left');

        $perpage = (int)$xformsHelper->getConfig('perpage'); // get # of forms to show per page
        ...
        if ($forms) {
            ...
            foreach ($forms as $f) {
                if ($showAll || $f->isActive()) {
                    $id    = $f->getVar('form_id');
                    $order = new XformsFormInput('', "order[{$id}]", 5, 5, $f->getVar('form_order'), null, 'number');
                    ...
                    if (XformsConstants::SEND_TO_OTHER == (int)$sendTo) {
                        $sendToTxt = '<b>' . _AM_XFORMS_SENDTO_OTHER . ': </b>' . $f->getVar('form_send_to_other');
                    } elseif ((XformsConstants::SEND_TO_NONE != $sendTo) && ($group = $group_mgr->get($sendTo))) {
                    ...
                    if (!$f->isActive()) {
                        if (XformsConstants::FORM_INACTIVE == $f->getVar('form_active')) {
                            ...
                            $fAction = " <a href='" . $_SERVER['PHP_SELF'] . "?op=active&form_id={$id}'><img src='{$mypathIcon16}/active.gif' class='tooltip floatcenter1' title='"
                        ...
                        $fAction = "<a href='" . $_SERVER['PHP_SELF'] . "?op=inactive&form_id={$id}'><img src='{$mypathIcon16}/inactive.gif' class='tooltip floatcenter1' title='"
                                   . _AM_XFORMS_ACTION_INACTIVE . "' alt='" . _AM_XFORMS_ACTION_INACTIVE . "'></a>";
                    }
                    $ids = new XoopsFormHidden('ids[]', $id);
                    echo "  <tr>\n" . "    <td class='odd middle center'>{$id}</td>\n" . "    <td class='even middle'><a href='" . $xformsHelper->url("index.php?form_id={$id}") . "' title='"
                         . _AM_XFORMS_ACTION_VIEWFORM . "'>" . $f->getVar('form_title', 's') . "</a><br>\n" . '      ' . $f->getVar('form_desc', 's') . "\n" . "    </td>\n"
                         ...
                         . "?op=edit&form_id={$id}'><img src='" . Admin::iconUrl('edit.png', '16') . "' class='tooltip floatcenter1' title='" . _AM_XFORMS_ACTION_EDITFORM . "' alt='"
                         . _AM_XFORMS_ACTION_EDITFORM . "'></a>\n" . "      {$fAction}\n" . "      <a href='" . $_SERVER['PHP_SELF'] . "?op=edit&clone=1&form_id={$id}'><img src='"
                         . Admin::iconUrl('editcopy.png', '16') . "' class='tooltip floatcenter1' title='" . _AM_XFORMS_ACTION_CLONE . "' alt='" . _AM_XFORMS_ACTION_CLONE . "'></a>\n"
                         . "      <a href='" . $_SERVER['PHP_SELF'] . "?op=delete&form_id={$id}'><img src='" . Admin::iconUrl('delete.png', '16') . "' class='tooltip floatcenter1' title='" . _DELETE
                         . "' alt='" . _DELETE . "'></a>\n" . '      ' . $ids->render() . "\n" . "      <a target='_blank' href='" . $xformsHelper->url("index.php?form_id={$id}") . "'><img src='"
                         . Admin::iconUrl('view.png', '16') . "' class='tooltip floatcenter1' title='" . _AM_XFORMS_ACTION_VIEWFORM . "' alt='" . _AM_XFORMS_ACTION_VIEWFORM . "'></a>\n";

                    if (XformsConstants::SAVE_IN_DB == $f->getVar('form_save_db') && isset($rptCountArray[$id])) {
                    ...
                    echo "      <a href='elements.php?form_id={$id}'><img src='" . Admin::iconUrl('1day.png', '16') . "' class='tooltip floatcenter1' title='" . _AM_XFORMS_ACTION_EDITELEMENT
                         . "' alt='" . _AM_XFORMS_ACTION_EDITELEMENT . "'></a>\n" . "    </td>\n" . "  </tr>\n";
        ...
        if (0 == $totalList) {
            $bshow = new XoopsFormButton('', ($showAll ? 'shownormal' : 'showall'), ($showAll ? _AM_XFORMS_SHOW_NORMAL_FORMS : _AM_XFORMS_SHOW_ALL_FORMS), 'submit');
            echo "  <tr>\n" . "    <td class='odd center' colspan='6'>" . _AM_XFORMS_NO_FORMS . "</td>\n" . "  </tr>\n" . "  </tbody>\n" . "  <tfoot>\n" . "  <tr>\n"
    ...
    case 'edit':
        ...
        $moduleAdmin = Admin::getInstance();
        $moduleAdmin->displayNavigation(basename(__FILE__) . '?op=edit');
        ...
        if (0 == $formId) {
            ...
            //            $groupIds      = $GLOBALS['modulepermHandler']->getGroupIds($xformsFormsHandler->perm_name, $formId, $xformsHelper->getModule()->getVar('mid'));
        }
        ...
        $selectFormSendMethod->addOption(XformsConstants::SEND_METHOD_MAIL, _AM_XFORMS_SEND_METHOD_MAIL);
        $selectFormSendMethod->addOption(XformsConstants::SEND_METHOD_PM, _AM_XFORMS_SEND_METHOD_PM);
        $selectFormSendMethod->addOption(XformsConstants::SEND_METHOD_NONE, _AM_XFORMS_SEND_METHOD_NONE);
        ...
        $editorConfigs        = array_merge($editorConfigs, array(
        ...
        $selectFormDelimiter->addOption(XformsConstants::DELIMITER_SPACE, _AM_XFORMS_DELIMETER_SPACE);
        $selectFormDelimiter->addOption(XformsConstants::DELIMITER_BR, _AM_XFORMS_DELIMETER_BR);

        $textFormOrder = new XformsFormInput(_AM_XFORMS_ORDER, 'form_order', 4, 5, $form->getVar('form_order'), null, 'number');
        ...
        $selectFormDisplayStyle->addOption(XformsConstants::FORM_DISPLAY_STYLE_FORM, _AM_XFORMS_DISPLAY_STYLE_FORM);
        $selectFormDisplayStyle->addOption(XformsConstants::FORM_DISPLAY_STYLE_POLL, _AM_XFORMS_DISPLAY_STYLE_POLL);
    ...
    case 'delete':
        if (empty($_POST['ok'])) {
            ...
            if ($formId) {
                ...
                redirect_header($_SERVER['PHP_SELF'], XformsConstants::REDIRECT_DELAY_MEDIUM, _AM_XFORMS_FORM_NOTEXISTS);
            }
        } else {
            ...
            if (!empty($formId) && ($formObj = $xformsFormsHandler->get($formId)) && !$formObj->isNew()) {
                if ($xformsFormsHandler->delete($formObj)) {
                    ...
                    redirect_header($_SERVER['PHP_SELF'], XformsConstants::REDIRECT_DELAY_NONE, _AM_XFORMS_DBUPDATED);
                }
                xoops_cp_header();
                echo $formObj->getHtmlErrors();
            } else {
                redirect_header($_SERVER['PHP_SELF'], XformsConstants::REDIRECT_DELAY_NONE, _AM_XFORMS_NOTHING_SELECTED);
            }
        }
        break;
    ...
    case 'active':
        if (empty($_POST['ok'])) {
            ...
            if ($formId) {
                ...
                $formObj            = $xformsFormsHandler->get($formId);
                $formTitle          = $formObj->getVar('form_title');
                xoops_confirm(array('op' => 'active', 'form_id' => $formId, 'ok' => 1), $_SERVER['PHP_SELF'], sprintf(_AM_XFORMS_CONFIRM_ACTIVE, $formTitle));
            } else {
                redirect_header($_SERVER['PHP_SELF'], XformsConstants::REDIRECT_DELAY_MEDIUM, _AM_XFORMS_FORM_NOTEXISTS);
            }
        } else {
    ...
    case 'saveorder':
        ...
        array_walk($order, 'XformsUtilities::intArray'); // can't use array_map since must preserve keys
        foreach ($ids as $id) {
    ...
    case 'saveform':
        ...
        if ((0 == $formSaveDb) && (XformsConstants::SEND_METHOD_NONE == $formSendMethod)) {
            redirect_header($_SERVER['PHP_SELF'], XformsConstants::REDIRECT_DELAY_NONE, _AM_XFORMS_NOTHING_SAVESENT);
        }
        ...
        if (!empty($error)) {
            ...
            if (_AM_XFORMS_SAVE_THEN_ELEMENTS == $_POST['submit']) {
                redirect_header($xformsHelper->url("admin/elements.php?form_id={$ret}"), XformsConstants::REDIRECT_DELAY_NONE, _AM_XFORMS_DBUPDATED);
            } else {
                redirect_header($_SERVER['PHP_SELF'], XformsConstants::REDIRECT_DELAY_NONE, _AM_XFORMS_DBUPDATED);
            }
        }
